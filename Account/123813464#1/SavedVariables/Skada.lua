
SkadaDB = {
	["namespaces"] = {
		["LibDualSpec-1.0"] = {
		},
	},
	["profileKeys"] = {
		["Вейлзевулла - Борейская тундра"] = "Default",
		["Алтери - Борейская тундра"] = "Default",
		["Эстрея - Борейская тундра"] = "Default",
		["Зонна - Борейская тундра"] = "Default",
		["Эрника - Борейская тундра"] = "Default",
		["Алоиза - Борейская тундра"] = "Default",
		["Тиаз - Борейская тундра"] = "Default",
		["Нирейя - Борейская тундра"] = "Default",
		["Ниоши - Борейская тундра"] = "Default",
		["Дэлери - Борейская тундра"] = "Default",
		["Элави - Борейская тундра"] = "Default",
		["Altery - Bronzebeard"] = "Default",
		["Фламиника - Борейская тундра"] = "Default",
		["Сайбил - Борейская тундра"] = "Default",
	},
	["profiles"] = {
		["Default"] = {
			["modulesBlocked"] = {
				["Debuffs"] = false,
				["CC"] = true,
				["Power"] = false,
				["Healing"] = false,
				["DamageTaken"] = false,
				["Themes"] = true,
				["Tweaks"] = true,
				["Friendly Fire"] = false,
				["Overhealing"] = false,
				["TotalHealing"] = false,
				["Deaths"] = false,
				["Enemies"] = false,
				["Threat"] = true,
			},
			["modeclicks"] = {
				["Получено урона врагом"] = 123,
				["Урон"] = 305,
				["Отрицательные эффекты"] = 2,
				["Произведено исцеления врагом"] = 1,
				["Избыточное лечение"] = 1,
				["Действие дебаффа"] = 2,
				["УВС"] = 1,
				["Смерти"] = 25,
				["Исцеление"] = 237,
				["Получено лечения"] = 4,
				["Yрон"] = 98,
				["Полученный урон"] = 29,
				["Нанесено урона врагом"] = 9,
				["Время действия баффов"] = 4,
				["Рассеивания"] = 3,
				["Прерывание"] = 12,
				["Урон, полученный от заклинания"] = 5,
			},
			["windows"] = {
				{
					["set"] = "total",
					["hidden"] = true,
					["y"] = -137.974502563477,
					["point"] = "RIGHT",
					["mode"] = "Урон",
					["barwidth"] = 261.000122070313,
					["background"] = {
						["height"] = 194.000045776367,
					},
					["x"] = -9.9998779296875,
				}, -- [1]
			},
			["report"] = {
				["channel"] = "instance_chat",
				["mode"] = "Урон",
			},
			["versions"] = {
				["1.6.7"] = true,
				["1.6.4"] = true,
				["1.6.3"] = true,
			},
			["setstokeep"] = 59,
			["reset"] = {
				["join"] = 1,
			},
		},
	},
}
